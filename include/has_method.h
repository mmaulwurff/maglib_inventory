...
class has_##method<C, Ret(Args...)> { \
    template <typename T> \
    static constexpr auto check(T*) -> typename std::is_same<decltype( \
        std::declval<T>().method(std::declval<Args>()...)), Ret>::type; \
\
    template <typename> \
...
has_##method<type, method_type>::value
